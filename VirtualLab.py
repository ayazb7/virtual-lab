...
class MenuWindow(Window):
    ...
    def _initUI(self):
        ...
        self.__option1Button = self.__initMenuButton("./img/manualButton.png",pagButton1)
        self.__option2Button = self.__initMenuButton("./img/plancksConstant.png",pagButton2)
...
class ChoiceWindow(Window):
    ...
    def _initUI(self):
        ...
        self.__choice1 = self.__initChoiceButton("./img/manualChoice1.png",1,self.__pagType)
        self.__choice2 = self.__initChoiceButton("./img/manualChoice2.png",2,self.__pagType)
...
class AnimationView(QGraphicsView):
    ...
    def drawBackground(self, painter, rect):
        ...
        bg = QPixmap("./img/background2.png")
...
class AnimationWindow(Window):
    ...
    def _initUI(self):
        ...
        self.nextButton.setIcon(QIcon("./img/nextArrow.png/"))
        ...
        self.prevButton.setIcon(QIcon("./img/prevArrow.png/"))
    ...
    def startAnimation(self,pagType, choice):
        ...
        if pagType == "Vertical":
            ...
            elif self.startButton.text() == "Pause Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 4):
                self.animView.pauseAnims()
                self.startButton.setText("Resume Animation")
            elif self.startButton.text() == "Resume Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 4):
                self.animView.resumeAnims()
                self.startButton.setText("Pause Animation")
            elif self.startButton.text() == "Start Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 4):
            ...
            elif self.startButton.text() == "Pause Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 3):
                self.animView.pauseAnims()
                self.startButton.setText("Resume Animation")
            elif self.startButton.text() == "Resume Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 3):
                self.animView.resumeAnims()
                self.startButton.setText("Pause Animation")
            elif self.startButton.text() == "Start Animation" and (self.animView.getAnimState() == 2 or self.animView.getAnimState() == 3):
                self.animView.startAnims()
                self.startButton.setText("Pause Animation")
    ...
    def calculations(self,pagType):
        if pagType == "Vertical":
            ...
            self.textBox.setText("Repeat experiment multiple times at different heights and record all measurements in a table")
        elif pagType == "Ramp":
            self.calcWindow = True
            self.animWindow = False
            self.startButton.setText("Start Example")
        ...
        if pagType == "Vertical":
            if not self.calcWindow and self.animWindow:
                ...
                if not self.calcBox.graphShown:
                    self.calcBox.graph()
                    self.textBox.setText("Draw a graph of Distance (y axis) against time\u00b2 (x axis), with a line of best fit")
...
class DIYWindow(Window):
    ...
    def _initUI(self):
        ...
        if self.pagType != "Planck":
            headers = ["Distance (m)","1: time\u00b2 (s\u00b2)","2: time\u00b2 (s\u00b2)","3: time\u00b2 (s\u00b2)","Average time\u00b2 (s\u00b2)"]
        else:
            headers = ["Wavelength (x 10^-9 m)","1: Voltage (V)","2: Voltage (V)","3: Voltage (V)","Average Voltage (V))"]
    ...
    def firstVariableEntered(self):
        try:
            ...
            if self.pagType != "Planck":
                ...
                self.errorLabel.setText("Error: Incorrect form entered, Enter wavelength as a number in x 10^-9 nanometres")
    ...
    def startAnimation(self, pagType, choice):
        ...
        if pagType == "Vertical":
            if self.startButton.text() == "Start Practical":
                ...
                self.textBox.setText("Measure the Distance of the Drop from the bottom of the ball and enter the time measured")
            ...
            if self.startButton.text() == "Start Practical":
                ...
                if self.circuitConnected:
                    ...
                    self.textBox.setText("Change the resistance of the variable resistor and measure the thresold voltage")
                else:
    ...
    def finalValueEntered(self):
        ...
        try:
            if self.pagType != "Planck":
                ...
                finalText = textSplit[0] + ", " + "Value for g: " + str(text2) + "m/s\u00b2" + ", " + "Percentage Error: " + str(finalError) + "%"
            else:
                error = abs(((6.63e-34 - float(text2))/6.63e-34)* 100)
                finalError = round(error,1)
                finalText = textSplit[0] + ", " + "Value for g: " + str(text2) + ", Percentage Error: " + str(finalError) + "%"
...
class RampFall(AnimScene):
    ...
    def createObjects(self):
        if self.choice == 1:
            self.ball = GraphicsObject("./img/ball.png")
            self.ramp = GraphicsObject("./img/ramp1.png")
        else:
            self.ball = MovableImage(40,40,"./img/ball.png")
            self.ruler = MovableImage(460,505,"./img/ruler2.png")
            self.ramp = GraphicsObject("./img/ramp1.png")
            pix = QPixmap("./img/ruler2.png")
...
class VerticalFall(AnimScene):
    ...
    def createObjects(self):
        if self.choice == 1:
            self.ball = GraphicsObject("./img/ball.png")
            self.ruler = GraphicsObject("./img/ruler.png")
        else:
            self.ball = MovableImage(40,40,"./img/ball.png")
            self.ruler = MovableImage(50,505,"./img/ruler2.png")
...
class PlanckAnim(AnimScene):
    ...
    def createObjects(self):
        if self.choice == 1:
            self.circuit = GraphicsObject("./img/circuit2.png")
            self.slider = GraphicsObject("./img/slider.png")
        else:
            self.supply = GraphicsObject("./img/supply.png")
            self.voltmeter = GraphicsObject("./img/voltmeter.png")
            self.vResistor = GraphicsObject("./img/resistor.png")
    ...
    def mouseReleaseEvent(self, event):
        if self.choice == 2 and not self.circuitConnected:
            ...
            for component in self.components:
                ...
                if (l.x1() >= startPos.x() and l.x1() <= endPos.x()) and (l.y1() >= startPos.y() and l.y1() <= endPos.y()):
                    ...
                    if (l.x2() >= validStartPos1.x() and l.x2() <= validEndPos1.x()) and (l.y2() >= validStartPos1.y() and l.y2() <= validEndPos1.y()):
                        component[length-2] += 1
                        validEndComponent1[length2-2] += 1
                    elif (l.x2() >= validStartPos2.x() and l.x2() <= validEndPos2.x()) and (l.y2() >= validStartPos2.y() and l.y2() <= validEndPos2.y()):
                        component[length-2] += 1
                        validEndComponent2[length3-2] += 1
                    elif (l.x2() >= validStartPos3.x() and l.x2() <= validEndPos3.x()) and (l.y2() >= validStartPos3.y() and l.y2() <= validEndPos3.y()):
                        component[length-2] += 1
...
class CalculationVertical(GraphCalcBox):
    ...
    def initView(self):
        ...
        self.data = {"Distance (m)":["0.25","0.50","0.75","1.00","1.25","1.50","1.75","2.00"],
                "1: time\u00b2 (s\u00b2)":["0.051","0.102","0.153","0.204","0.255","0.306","0.357","0.408"],
                "2: time\u00b2(s\u00b2)":["0.050","0.099","0.152","0.210","0.249","0.312","0.350","0.415"],
                "3: time\u00b2 (s\u00b2)":["0.048","0.103","0.156","0.216","0.258","0.307","0.363","0.413"],
                "Average time\u00b2 (s\u00b2)": ["0.050","0.101","0.154","0.210","0.254","0.308","0.357","0.412"]}
...
class CalculationRamp(QGroupBox):
    ...
    def initView(self):
        ...
        diagram = QLabel()
        pix = QPixmap("./img/rampDiagram1.png")
